
<!-- Wave Header -->

<!-- Typing Animation -->

<!-- Tech Icons -->
<p align="center">
  <img src="https://skillicons.dev/icons?i=python,mysql,fastapi,flask,pandas,numpy,opencv,git,github" />
</p>

<!-- Coding Workspace GIF -->
<p align="center">
  <img width="700" src="https://user-images.githubusercontent.com/74038190/240885693-897f0e09-4e8d-4e57-bd7b-8d9cbf87f6fd.gif">
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Dhanush%20Kanth%20T%20R&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=Data%20Science%20Engineer%20|%20Backend%20Developer%20|%20Competitive%20Programmer&descAlignY=62&descSize=16" />
</p>

<div align="center">

# 🚀 Data Science Engineer | Backend Developer | Competitive Programmer

### 729+ Problems Solved • 29 Contests • CodeChef 2★ • Python Specialist

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=900&lines=Data+Science+Engineer+📊;Backend+Developer+⚙️;Competitive+Programmer+🏆;729%2B+Problems+Solved;Building+Real-World+AI+Solutions;Open+to+Internships+%26+Collaborations+🚀" />

<br>

<a href="https://linkedin.com/in/dhanush-kanth-t-r-81ab28243">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" />
</a>

<a href="https://codolio.com/dk003">
  <img src="https://img.shields.io/badge/Portfolio-Codolio-70A5FD?style=flat-square&logo=googlechrome&logoColor=white" />
</a>

<a href="mailto:dhanushkanth03@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-EA4335?style=flat-square&logo=gmail&logoColor=white" />
</a>

<img src="https://komarev.com/ghpvc/?username=dk003&style=flat-square&color=70A5FD&label=Profile+Views" />

</div>

---

# 👋 About Me

<table>
<tr>
<td width="60%">

I'm a Computer Science undergraduate passionate about:

- 📊 Data Science & Machine Learning
- ⚙️ Backend Development
- 🏆 Competitive Programming
- 🧠 Data Structures & Algorithms
- 🚀 Real-world Product Development

I enjoy transforming data into insights, building scalable backend systems,
and continuously improving through coding contests and problem solving.

### 🎯 Career Objective

To contribute to innovative products by combining:

- Data Science
- Software Engineering
- Backend Development
- Competitive Programming

</td>

<td width="40%">

<img width="350" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif">

</td>
</tr>
</table>

---

# 👨‍💻 Competitive Programming Journey

## 🚀 Coding Philosophy

Strong software engineers are built through consistent problem solving.

I actively participate in coding contests and algorithmic challenges across
LeetCode, CodeChef, and Codeforces.

My goal is to reach:

- 1000+ Problems Solved
- CodeChef 3★
- Higher Contest Ratings
- Stronger DSA Expertise

---

# 📊 Coding Statistics Dashboard

| Metric | Achievement |
|----------|------------|
| 🧩 Problems Solved | 729+ |
| 📅 Active Coding Days | 248 |
| 🔥 Maximum Streak | 62 Days |
| 🏆 Contests Participated | 29 |
| 💛 LeetCode Rating | 1542 |
| 📈 Maximum LeetCode Rating | 1577 |
| ⭐ CodeChef Rating | 1432 (2★) |
| 🔵 Codeforces Rating | 427 |

---

# ⚡ Problem Solving Breakdown

| Category | Count |
|----------|----------|
| 🏆 Competitive Programming | 513 |
| 🧠 DSA Problems | 153 |
| 📚 Fundamentals | 61 |
| ✅ Total Solved | 729 |

---

# 🧠 Top DSA Topics

| Topic | Problems |
|--------|----------|
| Arrays | 74 |
| Strings | 27 |
| HashMap & Set | 26 |
| Two Pointers | 23 |
| Binary Search | 23 |
| Algorithms | 18 |
| Sorting | 17 |
| Linked Lists | 17 |
| Math | 15 |
| Dynamic Programming | 14 |

---

# 🏆 Contest Ratings

| Platform | Rating | Max Rating |
|-----------|---------|------------|
| LeetCode | 1542 | 1577 |
| CodeChef | 1432 | 1432 |
| Codeforces | 427 | 427 |

---

# 🏅 My Coding Awards

<table>
<tr>
<td>🥇 Solved 500 Problems</td>
<td>🐍 Python 5-Star Badge</td>
<td>🗄️ SQL Badge</td>
</tr>

<tr>
<td>🏆 Participated in 25+ Contests</td>
<td>💎 100-Day Coding Streak</td>
<td>✍️ Explained 10 Solutions</td>
</tr>

<tr>
<td colspan="3" align="center">
📅 50 Days Badge 2025
</td>
</tr>
</table>

---

# 🛠️ Tech Stack

## Languages

- Python
- SQL

## Backend Development

- FastAPI
- Flask

## Data Science

- Pandas
- NumPy
- scikit-learn

## Database

- MySQL

## Visualization & Deployment

- Streamlit

## Computer Vision

- OpenCV

---

# 💼 Cognizant Experience

## Vehicle Insurance Claim Automation System

### Responsibilities

- Developed REST APIs using FastAPI
- Integrated OCR using Tesseract
- Built OpenCV damage detection module
- Connected MySQL database
- Implemented automated claim workflow

### Impact

- Reduced manual claim review effort
- Automated document extraction
- Improved processing efficiency

---

# 🚀 Featured Projects

## 🚗 Vehicle Claim Automation

**Stack**

Python • FastAPI • OpenCV • OCR • MySQL

### Highlights

- End-to-end insurance workflow
- Damage detection
- OCR extraction
- Automated claim processing

---

## 🌾 Crop Recommendation System

**Stack**

Python • Streamlit • Pandas • scikit-learn

### Highlights

- Soil-based recommendations
- ML-powered predictions
- Farmer-friendly UI

---

## 💳 Credit Scoring Platform

**Stack**

Python • Flask • MySQL

### Highlights

- Dynamic scoring model
- Financial accessibility focus
- Backend API architecture

---

## 🎬 Movie Ticket Booking System

**Stack**

Python • MySQL

### Highlights

- Seat allocation
- CRUD operations
- Booking management

---

# 📈 Professional Strengths

### Data Science

- Machine Learning
- Data Analysis
- Feature Engineering
- Predictive Modeling

### Backend Development

- REST APIs
- FastAPI
- Flask
- Database Integration

### Competitive Programming

- DSA
- Algorithms
- Problem Solving
- Contest Participation

---

# 🎓 Education

| Degree | Institution | Duration |
|----------|------------|------------|
| B.E. Computer Science & Engineering | Sathyabama Institute of Science & Technology | 2023–2027 |

---

# 📚 Currently Learning

### Machine Learning

- Model Deployment
- MLOps
- Feature Engineering

### Backend Development

- Advanced FastAPI
- Authentication
- Async APIs

### System Design

- Scalability
- Database Optimization
- API Architecture

### Competitive Programming

- Advanced DSA
- Dynamic Programming
- Contest Preparation

---

# 🎯 2026 Goals

- Reach 1000+ Problems Solved
- Achieve CodeChef 3★
- Build Production-grade AI Applications
- Secure Data Science Internship
- Strengthen System Design Skills

---

# 🌟 Why Recruiters Should Consider Me

✅ Strong Problem Solving Skills

✅ Competitive Programming Background

✅ Real-world Project Experience

✅ Data Science Knowledge

✅ Backend Development Expertise

✅ Continuous Learner
---

# 🐍 Contribution Snake

<div align="center">

<img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg" alt="Snake animation" />

</div>

---

---

# 📬 Let's Connect

### LinkedIn

https://linkedin.com/in/dhanush-kanth-t-r-81ab28243

### Portfolio

https://codolio.com/dk003

### Email

dhanushkanth03@gmail.com

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer&animation=twinkling" />
</p>

<div align="center">

### ⭐ Thanks for visiting my profile!

Building solutions, solving problems, and learning every day.

</div>
